...
let pizzaVieja = JSON.parse(localStorage.getItem("pizza")) || [];

const saveLocalStorage = (pizzaVieja) => {
  localStorage.setItem("pizza", JSON.stringify(pizzaVieja));
};
console.log(pizzaVieja);
...
const renderPizza = () => {
  ...
  //<img src="${pizza.id}.jpg" alt=Pizza de ${pizza.nombre}>
  return ` 
...
//no se como hacerlo en una sola funcion

const renderPizzaVieja = () => {
  const pizza = pizzaVieja;
...
const renderCarritoViejo = () => {
  pizzaList.innerHTML = renderPizzaVieja();
};
